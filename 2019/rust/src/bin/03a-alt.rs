// simplify version of how to calc intersection of line segment by reduce line to only horizontal and vertical
...
fn main() {
    let a_directions = move2points(parse_move(A_MOVE));
    let b_directions = move2points(parse_move(B_MOVE));
    let intersects = intersect(a_directions, b_directions);
    let distance = intersects
        .into_iter()
        .map(|x| manhattan_distance(Point::new(0, 0), x))
        .min()
        .unwrap();
    println!("closest: {}", distance);
}

fn intersect(a: Vec<Direction>, b: Vec<Direction>) -> Vec<Point> {
    let mut res = Vec::new();
    for i in 0..a.len() {
        for j in 0..b.len() {
            if let Some(intersect) = is_line_intersect(a[i], b[j]) {
...
fn is_line_intersect(a: Direction, b: Direction) -> Option<Point> {
    let (x, x0, x1, y, y0, y1) = match (a, b) {
        (Direction::Horizontal(y, x0, x1), Direction::Vertical(x, y0, y1)) => {
            (x, x0, x1, y, y0, y1)
        }
        (Direction::Vertical(x, y0, y1), Direction::Horizontal(y, x0, x1)) => {
            return is_line_intersect(b, a);
        }
        _ => return None, // same direction is never intersect
    };
...
fn move2points(moves: Vec<Move>) -> Vec<Direction> {
    ...
    for mv in moves {
        ...
        match mv {
            Move::L(n) => {
                d = Direction::Horizontal(cur.y, cur.x, cur.x - n);
                cur += Point::new(-n, 0);
            }
            Move::R(n) => {
                d = Direction::Horizontal(cur.y, cur.x, cur.x + n);
            ...
            Move::U(n) => {
                d = Direction::Vertical(cur.x, cur.y, cur.y + n);
                cur += Point::new(0, n);
            }
            Move::D(n) => {
                d = Direction::Vertical(cur.x, cur.y, cur.y - n);
