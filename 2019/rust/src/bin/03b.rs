...
fn main() {
    let a_points = move2points(parse_move(A_MOVE));
    let b_points = move2points(parse_move(B_MOVE));
    let r = sum_intersect_step(a_points, b_points)
        .into_iter()
        .min()
        .unwrap();
    println!("fewest: {}", r);
...
fn sum_intersect_step(a: Vec<Point>, b: Vec<Point>) -> Vec<i64> {
    let mut res = Vec::new();
    for i in 0..a.len() - 1 {
        for j in 0..b.len() - 1 {
            if let Some(intersect) = is_line_intersect(a[i], a[i + 1], b[j], b[j + 1]) {
                if intersect.x == 0 && intersect.y == 0 {
                    continue;
                }

                let mut a_step = manhattan_distance(a[i], intersect);
                let mut b_step = manhattan_distance(b[j], intersect);

                // sum a & b by walking from start to here
                for k in 0..i {
                    a_step += manhattan_distance(a[k], a[k + 1]);
                }

                for k in 0..j {
                    b_step += manhattan_distance(b[k], b[k + 1]);
                }
                res.push(a_step + b_step);
            }
        }
    }
    res
...
// steal and convert to rust from http://paulbourke.net/geometry/pointlineplane/javascript.txt
fn is_line_intersect(a1: Point, a2: Point, b1: Point, b2: Point) -> Option<Point> {
    ...
    let denominator = ((b2.y - b1.y) * (a2.x - a1.x) - (b2.x - b1.x) * (a2.y - a1.y)) as f64;
    if denominator == 0.0 {
        return None;
    }

    let ua = ((b2.x - b1.x) * (a1.y - b1.y) - (b2.y - b1.y) * (a1.x - b1.x)) as f64 / denominator;
    let ub = ((a2.x - a1.x) * (a1.y - b1.y) - (a2.y - a1.y) * (a1.x - b1.x)) as f64 / denominator;
    ...
    let x = a1.x as f64 + ua * (a2.x - a1.x) as f64;
    let y = a1.y as f64 + ua * (a2.y - a1.y) as f64;
    Some(Point::new(x as i64, y as i64))
}

fn move2points(moves: Vec<Move>) -> Vec<Point> {
    ...
    for mv in moves {
        ...
        match mv {
            Move::L(n) => {
                to = Point::new(cur.x - n, cur.y);
            }
            Move::R(n) => {
                to = Point::new(cur.x + n, cur.y);
            }
            Move::U(n) => {
                to = Point::new(cur.x, cur.y + n);
            }
            Move::D(n) => {
                to = Point::new(cur.x, cur.y - n);
            }
...
fn parse_move(seq: &str) -> Vec<Move> {
    seq.split_terminator(",")
        .map(|x| match x.chars().nth(0).unwrap() {
            'L' => Move::L(x[1..].parse::<i64>().unwrap()),
            'R' => Move::R(x[1..].parse::<i64>().unwrap()),
            'U' => Move::U(x[1..].parse::<i64>().unwrap()),
            'D' => Move::D(x[1..].parse::<i64>().unwrap()),
            _ => panic!("unreachable"),
        })
        .collect()
}
